...
def ensure_virtual_trades():
    ...
    for event in get_signal_events_without_trades():
        entry_price = float(event["entry_price"])
        atr = float(event["atr"])
        signal = event["signal"]

        atr_stop_distance = atr * STOP_LOSS_ATR_MULTIPLIER
        minimum_stop_distance = MIN_STOP_PIPS * PIP_SIZE
        ...
        created, trade_id = save_virtual_trade(
            ...
            max_hold_minutes=MAX_TRADE_MINUTES,
        )
...
def evaluate_open_trades(candles):
    ...
    for trade in get_open_virtual_trades():
        signal_candle_open = datetime.strptime(
        ...
        maximum_exit_time = (
            actual_entry_time
            + timedelta(
                minutes=trade["max_hold_minutes"]
            )
        )
        ...
        for candle in candles:
            ...
            if stop_hit and target_hit:
                closed = close_virtual_trade(
                    trade_id=trade["id"],
                    status="AMBIGUOUS",
                    exit_candle_time=candle["datetime"],
                    exit_price=None,
            ...
            if candle_close_time >= maximum_exit_time:
                gross_pips = calculate_directional_pips(
                    signal=signal,
                    entry_price=entry_price,
                    exit_price=close,
                )
