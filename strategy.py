...
def analyze_market(candles):
    ...
    if candidate == "BUY":
        checks = {
            "volatility": atr_value >= MIN_ATR,
            "ema_separation": ema_distance_atr >= MIN_EMA_DISTANCE_ATR,
            "ema_slope": ema_direction == "UP",
            "price_position": close > ema_fast and close > ema_slow,
            "rsi": RSI_BUY_MIN <= rsi_value <= RSI_BUY_MAX,
        }

        blocker_messages = {
            "volatility": "volatility too low",
            "ema_separation": "weak EMA separation",
            "ema_slope": "EMAs are not rising together",
            "price_position": "price is not above both EMAs",
            "rsi": "RSI does not confirm BUY",
        }
        ...
        blocker_messages = {
            "volatility": "volatility too low",
            "ema_separation": "weak EMA separation",
            "ema_slope": "EMAs are not falling together",
            "price_position": "price is not below both EMAs",
            "rsi": "RSI does not confirm SELL",
        }
    ...
    if blockers:
        ...
        reason = "All strategy filters passed"
